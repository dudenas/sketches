  ...
  for (let particle of particles) {
    ...
    if (len > minLenPath) {
      ...
      for (let i = 0; i < len - 2; i++) {
        ...
        // grfcCanvas.line(xa * res, ya * res, xb * res, yb * res);
        if (Math.abs(ya - yb) < 10) {
          // A
          // for (let j = 0; j < interpolate; j++) {
          //   let x = map(j, 0, interpolate - 1, xa, xb)
          //   let y = map(j, 0, interpolate - 1, ya, yb)
          //   let sat = map(j, 0, interpolate - 1, aValue[0], bValue[0])
          //   let bri = map(j, 0, interpolate - 1, aValue[1], bValue[1])
          //   grfcCanvas.fill(saveClrs[currClr][0], sat, bri);
          ...
          // grfcCanvas.vertex(xa * res, ya * res);
          // grfcCanvas.vertex(xb * res, yb * res);
          // grfcCanvas.endShape()
          // C
          grfcCanvas.strokeWeight(r * res)
          grfcCanvas.noFill()
