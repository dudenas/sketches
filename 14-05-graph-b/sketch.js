...
function draw() {
	if (situation) {
		...
		if (individualSituation) {
			individualSituationDraw()
		} else {
			let indvElmnts = document.getElementsByClassName('currQuestion-div')
			for (let i = 0; i < indvElmnts.length; i++) indvElmnts[i].setAttribute('style', 'display:none')
			situationDraw()
		}
	} else {
		let infElmnts = document.getElementsByClassName('information')
